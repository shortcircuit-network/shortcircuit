...
// DDNSRecord represents one Cloudflare AAAA record
...
func main() {
	...
	ttl := 120
	if os.Getenv("TTL") != "" {
		fmt.Sscanf(os.Getenv("TTL"), "%d", fmt.Sscanf(os.Getenv("TTL"), "%d", log.Printf("Using TTL=%d", ttl)ttl)ttl)
	...
	if currentIP == "" {
		log.Fatal("could not obtain public IPv6")
	}
	...
	if currentIP == lastIP {
		log.Printf("IPv6 unchanged (%s), skipping update", currentIP)
		return
	}

	// Update all records
	for _, rec := range records {
		if err := updateCloudflareAAAA(rec, currentIP, cfToken, ttl); err != nil {
...
func getPublicIPv6() string {
	providers := []string{"https://6.ifconfig.me", "https://api64.ipify.org"}
...
// Cloudflare AAAA update
func updateCloudflareAAAA(rec DDNSRecord, ipv6, token string, ttl int) error {
	...
	if r.Success == false && len(r.Errors) == 0 {
		r.Errors = []struct{Code int; Message string}{{0,"empty response"}}
	}
	if !r.Success {
